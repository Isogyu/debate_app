...
> - `JUDGING.md` — 審査要項から抜き出した実装要件と、その対応状況
...
- **分量: 読み上げ5分。超過しても、30秒以上余らせても減点される（絶対要件）**
  - 適正は **4分30秒超〜5分00秒** の30秒幅しかない（審査要項5。詳細は `docs/JUDGING.md`）
  - 速度で帳尻を合わせるのは不可。早口・遅すぎも減点対象（審査要項11）
  - 実物の立論は本文1,528字・1,577字（`docs/samples/`）。これが5分に収まる長さの実測値
  - ここから逆算した既定の読み上げ速度は **320字/分**、持ち分は **1,600字**
  - 速さは個人差が大きいので設定で変更できる（260〜380字/分）
  - **分量は品質そのもの**。長く詳しい立論は、超過する時点で悪い立論である
  - 資料の参照記法（`【資料N参照】`）は発話しないので字数に数えない
...
検索語を入れた状態で開けるものは、トップページではなく**結果一覧へ直接飛ばす**
（打ち直させない）。ただし**実際に開いて絞り込まれることを確認したものだけ**に限る。
国会会議録のようにURLへ検索語を載せても効かない（全件が出る）サイトがあり、
壊れたリンクはトップページより悪い。確認済み: CiNii Research、e-Stat。
...
| 学術 | **CiNii Research**（法解釈・原則の定義・制度趣旨・判例研究。政策ディベートでは統計だけでは「なぜそう解釈すべきか」を支えられないため、法的論証の中核になる）、J-STAGE |
| 書籍 | 専門書（金子宏『租税法』等） |
...
| 扱うお題の数 | **常に1つ**。1つの論題に対していろいろな観点の立論を作るのがこのアプリの使い方。新しいお題を登録すると前のお題は「過去のお題」に片付く（消えない） |
| 練習回数 | 多いほどよい |
...
| U2 | 立論ビュー・編集 | 実フォーマット(Ⅰ主張/Ⅱ理由/Ⅲ結論)で生成。Claimカード編集・部分再生成。**読み上げ時間を常時表示し、5分超過を警告** | Must |
...
#### A（few-shot）の実装状況（v5で実装済み）

`src/domain/exemplars.ts` に、**実際の試合で使われた立論2件**（肯定側・否定側）を埋め込み、
生成する側に合ったものを立論の骨子・本文の生成時に提示している。

お手本は生成物ではなく本物である点に価値がある。構成・一文の密度・分量のすべてが
実戦の基準を満たしており、特に**読み上げ5分に収まる長さの実例**として働く。

内容を真似させないことが重要。お手本は別の論題のものなので、論点まで引きずられると
的外れな立論になる。プロンプトで「真似るもの（構成・密度・分量）」と
「真似てはいけないもの（論点・法令・用語）」を明示的に分けている。
