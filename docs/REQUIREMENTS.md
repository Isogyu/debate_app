...
| 層 | 機能 | 判断基準 |
|----|------|----------|
| **MVP（これだけで試合に出られる）** | U1 論題分析 / U2 立論 / U3 資料要件 / U8 エクスポート / U14 ブロック集 / U16 本番モード / U18 本番パック | 準備して試合に出るという一連の流れが成立する最小集合 |
| **第2陣（質を上げる）** | U5 反駁 / U4 質疑フロー / U6 比較衡量 / U7 プロジェクト管理の履歴 / U15 横断検索 / A1 バリエーション生成 | 勝率に効くが、なくても試合はできる |
| **後回し** | U9〜U13 / U17 / U19 / U20 / A2〜A6 | 余力があれば |
...
| 要素 | 本アプリでの対応 |
|------|------------------|
| **機密性** | APIキーはサーバー環境変数のみでクライアントに返さない。**プロジェクト単位のパスコード**（チームで1つ共有・ハッシュ保存）でアクセス制御し、他チームから分離する。立論内容は大会前の**競技上の秘匿情報**であるため、URLを知るだけでは閲覧できない状態にする |
...
| **真正性** | **チーム単位＝プロジェクトパスコードで担保**、**個人単位＝名前を選ぶだけの簡易識別**（ゼミ規模では個人ごとの厳格なパスワード運用は現実的でない）。管理画面は別途パスコード保護 |
| **責任追跡性** | 操作ログ（生成・編集・エクスポート）を記録し、誰がいつ何をしたか追跡可能 |
| **否認防止** | ログは追記型で保存（UPDATE/DELETEしない）。ただし個人識別が名前選択である以上、**否認防止が成立するのはチーム単位まで**で、個人単位は参考値であることを明示する（できない保証をしない） |
...
9. **大会規程上のAI利用可否**（未確認）: AI生成の立論をそのまま大会に提出してよいかは大会要項・ゼミの方針次第。本アプリは「準備支援」と位置づけるが、成果物がそのまま提出物になりうる → **運用開始前に顧問・大会要項で確認すること**。アプリ側は「AI生成／人が確認済み」のステータスを画面とエクスポートの両方に必ず表示する
...
12. **スケジュール未定**: 大会日程が本書に記載されていない。**MVP（§5の3層）の完成期限を大会日から逆算して確定させること**。期限が不明なままだと機能を削る判断ができない
