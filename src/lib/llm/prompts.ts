...
【厳守】データに基づく論証
立論全体で、統計資料（kind: "statistic"）を**2〜3個**含めてください。
推移（制定当時と現在の比較など）・割合・規模の比較のように、数値で示すと説得力が増す命題を選び、
その命題を「〜は大きく減少している」「〜の割合は高い」のような、統計表で確かめられる形で書きます。
（何を統計で示すかが決まると、アプリが e-Stat から実際の数値を取り、表・グラフ・計算過程を作ります）
`.trim();
