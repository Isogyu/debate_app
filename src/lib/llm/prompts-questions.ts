...
- 連鎖では、各質問に認める（admit）と否定する（deny）の分岐を必ず用意し、必要ならはぐらかす（evade）も加える。
  どう答えられても goal に向かって追及が続くようにする。**1つの連鎖は最大2段（起点＋追及）・5ノードまで**
...
【文字数の上限（出力が長すぎると途中で切れて全部使えなくなります）】
- question: 60字以内 / purpose: 30字以内 / goal: 40字以内
- modelAnswer: 80字以内 / expectedAnswer: 40字以内 / exposedWeakness: 40字以内（不要なら省略）
`.trim();
...
export function closingPrompt(ctx: {
  ...
  weaknesses: string[];
  /** own = この立論で戦うチーム用 / opponent = この立論と戦うチーム用。1回に1つだけ作る */
  perspective: "own" | "opponent";
}): string {
  const who =
    ctx.perspective === "own"
      ? `${ctx.sideLabel}＝この立論で戦うチームの最終弁論。
質疑で立論の柱が守られたこと、相手の攻撃が当たらなかった理由、相手立論との比較を、空欄に差し込む形にする`
      : `${ctx.opponentSideLabel}＝この立論と戦うチームの最終弁論。
質疑で相手が認めたこと・答えられなかったことを空欄に差し込み、それが相手立論のどこを崩すかを示す形にする`;
  return `
最終弁論（1分・約320字）の雛形を作ってください。
...
【厳守】
- frame は約320字（1分）。空欄は【①相手が認めたこと】のように【番号＋何を書くか】で示す。空欄は3〜4個
- blanks には空欄ごとの説明と、何を書けばよいかのヒント（ヒントは40字以内）
- examples は、上の連鎖の主な経路（認めた場合・否定した場合など）ごとに**2個**。空欄を埋めた完成文で、各300〜340字
